...
algW (UMH.V x) vars = (Assertion ([(x, (newVType vars))], MH.V x, (newVType vars)), union vars [(newVType vars)])
...
{-algW (UMH.Fun x e1) vars = if umg == []
                           then error "Unification Fails."
                           else (Assertion(ctx', MH.Fun x t' e1', t'), vars) where
  (Assertion(ctx1, ex1, t2), vars) = algW e1 vars
  nT = newVType vars2
  umg = µ[(t', t2)]
  ctx' = (sustCtx ctx1 umg)
  e1' = sustLT ex1 umg
  t' = sustTT t2 umg
  vars2 = union vars2 [nT]
-}
